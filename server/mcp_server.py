...
Exposes Android device control as MCP tools so that external agents
(e.g. Claude Code) can drive the observe→act loop themselves, without
DroidRun running an internal LLM.

Usage:
    python server/mcp_server.py                          # stdio transport (default)
    python server/mcp_server.py --transport sse           # SSE transport
    DEVICE_SERIAL=localhost:5555 python server/mcp_server.py
"""
...
logger = logging.getLogger("droidrun-mcp")
...
async def _ensure_ready() -> tuple[AndroidDriver, AndroidStateProvider]:
    ...
    global _driver, _state_provider, _ready
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DroidRun MCP Server")
    parser.add_argument(
        "--transport",
        choices=["stdio", "sse"],
        default="stdio",
