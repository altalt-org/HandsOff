...
aws ec2 describe-images \
  --owners 099720109477 \
  --filters "Name=name,Values=ubuntu/images/hvm-ssd-arm64/ubuntu-*-24.04-arm64-server-*" \
...
```bash
sudo apt-get update
sudo apt-get install -y docker.io docker-compose-plugin linux-modules-extra-$(uname -r) lzip python3-pip
